...
const CourtListScreen = ({ navigation, route }) => {
  const [courts, setCourts] = useState([]);
  const [loading, setLoading] = useState(true);
  const [userRole, setUserRole] = useState(null);

  useEffect(() => {
    const fetchCourtsAndRole = async () => {
      try {
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity
      style={styles.card}
      onPress={() => navigation.navigate('CourtDetails', { court: item, userRole })}
      activeOpacity={0.92}
    >
      <Image
      ...
      <View style={styles.cardContent}>
        ...
        <Text style={styles.courtSub}>⚽ Futsal / Indoor Court</Text>
        <View style={styles.priceRow}>
          <View>
            <Text style={styles.priceLabel}>Day rate</Text>
            <Text style={styles.price}>LKR {item.dayPrice}<Text style={styles.perUnit}> /hr</Text></Text>
          </View>
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.headerTitle}>⚽ Futsal Courts</Text>
        <Text style={styles.headerSub}>{courts.length} court{courts.length !== 1 ? 's' : ''} available</Text>
      </View>
      {courts.length === 0 ? (
        <View style={styles.emptyState}>
          <Text style={styles.emptyIcon}>⚽</Text>
          <Text style={styles.emptyTitle}>No courts available</Text>
          <Text style={styles.emptyText}>Check back later for available courts.</Text>
        </View>
      ) : (
        <FlatList
          data={courts}
      ...
      {userRole === 'admin' && (
        <TouchableOpacity style={styles.fab} onPress={() => navigation.navigate('CourtForm')}>
...
const styles = StyleSheet.create({
  ...
  courtName: { fontSize: 20, fontWeight: '700', color: '#1a2340', marginBottom: 4 },
  courtSub: { fontSize: 13, color: '#6b7c93', marginBottom: 14 },
